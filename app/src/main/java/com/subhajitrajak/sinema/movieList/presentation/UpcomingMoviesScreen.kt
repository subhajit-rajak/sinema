...
fun UpcomingMoviesScreen(
    ...
    if (movieListState.upcomingMovies.isEmpty()) {
        ...
        LazyVerticalGrid(
            ...
            items(movieListState.upcomingMovies.size) { index ->
                MovieItem(
                    movie = movieListState.upcomingMovies[index],
                    navHostController = navController
                )

                Spacer(modifier = Modifier.height(16.dp))

                if(index >= movieListState.upcomingMovies.size-1 && !movieListState.isLoading) {
